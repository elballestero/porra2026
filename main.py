...
def main(page: ft.Page):
    ...
    # Esta línea es la que hace la magia en la web
    page.assets_dir = "assets"

    page.fonts = {
        "Retro8Bit": "PressStart2P.ttf" 
    }
    ...
    def ir_a_precios(_): 
        ...
        page.add(
            ...
            ft.Text("ELIGE TUS EQUIPOS", size=18, font_family="Retro8Bit", color="yellow"),
            ft.Text("70 puntos para 7 selecciones", size=12, font_family="Retro8Bit", color="grey400"),
            ft.Divider(height=10, color="transparent")
        )
        ...
        try:
            ...
            for _, row in df_sorted.iterrows():
                # En la web, simplemente pasamos el nombre del archivo
                img = ft.Image(src=row['BANDERA'], width=30) 
        ...
        try:
            ...
            for _, row in df_sorted.iterrows():
                ...
                tile = ft.ExpansionTile(
                    title=ft.Row([
                        ft.Image(src=row['BANDERA'], width=25), # Eliminado os.path.exists
    ...
    def ir_a_clasificacion(_):
        ...
        try:
            ...
            for _, p in df_p.iterrows():
                ...
                ranking_data.append({"NOMBRE": p['NOMBRE'], "TOTAL": total_puntos, "EQUIPOS": equipos_p})
            ...
            for pos, p in enumerate(ranking_sorted, 1):
                ...
                for cod in p['EQUIPOS']:
                    ...
                    fila_banderas.controls.append(
                        ft.Container(
                            content=ft.Image(src=bandera_img, height=22, fit=ft.BoxFit.CONTAIN),
                            width=ancho_columna, alignment=ft.Alignment(0, 0)
                        )
                    )
                    fila_puntos.controls.append(
                        ft.Container(
                            content=ft.Text(str(pts_equipo), size=10, font_family="Retro8Bit", color="yellow"),
                            width=ancho_columna, alignment=ft.Alignment(0, 0)
                        )
                    )
                ...
                tile = ft.ExpansionTile(
                    ...
                    controls=[
                        ft.Container(
                            padding=ft.Padding(0, 15, 0, 20), bgcolor="#111111",
                            content=ft.Column([fila_banderas, ft.Divider(height=2, color="transparent"), fila_puntos], spacing=0)
                        )
                    ]
                )
                lista.controls.append(tile)

            page.add(ft.Row([btn_volver]), ft.Text("RANKING PORREROS", font_family="Retro8Bit", size=20, color="yellow"), lista)
    ...
    def ir_a_inicio(_=None):
        ...
        # Eliminado os.path.exists para el logo
        logo = ft.Image(src="FONDO.jpg", width=420, fit=ft.BoxFit.CONTAIN)
        
        btn_precios = ft.Container(
            content=ft.Row([ft.Text("ELIGE TUS 7", font_family="Retro8Bit", size=16, color="black", weight=ft.FontWeight.BOLD)], alignment="center"),
            bgcolor="yellow", width=250, height=60, border_radius=2,
            border=ft.Border(ft.BorderSide(4, "yellow"), ft.BorderSide(4, "yellow"), ft.BorderSide(4, "#333333"), ft.BorderSide(4, "#333333")),
        ...
        page.add(
            ft.Column(
                controls=[
                    ...
                    ft.Text("LA PORRA\nDEL MUNDIAL\n2026", font_family="Retro8Bit", size=24, color="yellow", text_align=ft.TextAlign.CENTER),
